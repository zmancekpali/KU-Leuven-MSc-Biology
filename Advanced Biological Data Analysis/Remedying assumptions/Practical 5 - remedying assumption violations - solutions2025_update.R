##########################################################
# EXERCISES PRACTICAL 5: REMEDYING ASSUMPTION VIOLATIONS #
##########################################################

#set your own working directory!
dir <- "C:/Users/pvdbe/GDrive/Teaching/Advanced Biological Data Analysis/Aanpassingen 2024/Practical 5 - remedying assumption violations/"
setwd(dir)


#for this exercise, make sure you have a former version of the package "car" as the latest version causes some issues
...
d = read.xlsx("blood_glucose.xlsx", sheet=1)
head(d)
str(d)

par(mfrow = c(1,2))
plot(blood_glucose ~ carb_consumption, data=d)
plot(blood_glucose ~ weekly_exercise, data=d)
par(mfrow = c(1,1))
...
fit1 <- lm(blood_glucose ~ weekly_exercise * carb_consumption, data = d)
...
summary(fit1b) #still no significant interaction
...
#the effects plot shows that the slope fot the medicine is more downward than
